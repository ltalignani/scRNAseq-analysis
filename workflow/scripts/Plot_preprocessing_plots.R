...
for (i in 1:length(seurat_obj)) {
  ...
  # Plot variable features without and with labels
  plot1 <- VariableFeaturePlot(seurat_obj[[i]]) +
    theme(legend.position = "none") # Supprime la légende du premier plot
  ...
  # Combinaison des deux plots
  # variable_features[[i]] <- plot1 + plot2
  variable_features[[i]] <- plot2
